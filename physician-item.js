window.addEventListener("DOMContentLoaded", function() {
    ...
    function searchPhysicians(specCode) {
        ...
        let request = sendRequest("GET", `organizations/healthcare-services/slots?criteria_values=${specCode}&slots_before=${slots_before}&slots_after=${slots_after}`, null);
    ...
    function populateTimetable(results) {
        results.forEach((doc, i) => {
            ...
            card.querySelector("#physician-fee").innerText = doc.price?.amount_formatted + "/h" || "-/h";
            ...
            if (doc && doc.managing_user && doc.managing_user.practitioner) {
                ...
                if (doc.managing_user.practitioner.description) {
                    isBioBtn.style.display = 'flex';
            ...
            doc.slots.forEach((slot, index) => {
                ...
                slotColumn.innerHTML = `<div class='slots_day-header'>
	  ...
	  <div class="slots_day-hours">`;
            ...
            let slider = tns({
                ...
                rewind: false,
                loop: false,
                controlsText: ['<div class="slots_arrow-wrapper btn-left"><img src="https://uploads-ssl.webflow.com/628be9abfa77a2e744cdac5f/62c81523d5c3fbbacd686126_expand_circle_left.svg" loading="lazy" alt=""></div>', '<div class="slots_arrow-wrapper btn-right"><img src="https://uploads-ssl.webflow.com/628be9abfa77a2e744cdac5f/62c8152412be42ad04db4bf0_expand_circle_right.svg" loading="lazy" alt=""></div>']
            });
    ...
    function getItemsCount(slider) {
        ...
        for (let item of info.slideItems[info.index].children) {
            itemsCount = itemsCount > item.children[1].childElementCount ? itemsCount : item.children[1].childElementCount
        }
        return itemsCount
    }
    ...
    function changeContainerHeight(slideContainer, slider) {
        ...
        if (!slideContainer.classList.contains("hideContent")) {
            const itemsCount = getItemsCount(slider);
            const containerHeight = itemHeight > 50 ? itemsCount * (itemHeight + 11) + 65 : itemsCount * (itemHeight + 8) + 65;
            slideContainer.style.height = containerHeight < defaultContainerHeight ? defaultContainerHeight : `${containerHeight}px`
        } else {
